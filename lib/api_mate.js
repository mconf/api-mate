...
(function() {
  var ApiMate, bindTooltips, isFilled, pad, postErrorTemplate, postSuccessTemplate, post_template_file, post_template_url, template;

  template = "<div class='result-set'>     <div class='result-title'>       <h4 class='label-title'>Results {{title}}:</h4>     </div>     <div class='result-links'>       {{#urls}}         <div class='result-link-wrapper'>         <div class='result-link {{urlClass}}'>           <a href='#' data-url='{{url}}' class='api-link-post tooltipped label' title='Send \"{{name}}\" using a POST request'>             post           </a>           <span class='method-name'>{{name}}</span>           <a class='api-link' href='{{url}}'>{{urlName}}</a>         </div>         </div>       {{/urls}}     </div>   </div>";
