# declaration of local variables that are defined at the bottom of the file
resultsTemplate = ''
postSuccessTemplate = ''
postErrorTemplate = ''
preUploadUrl = ''
...
class ApiMate
  ...
  start: ->
    ...
    $("input, select, textarea", "#menu-inputs").on "change keyup", (e) =>
      @generateUrls()
      @addUrlsToPage(@urls) if @getCurrentTab() is 'all'
    $("input, select, textarea", "#menu-server").on "change keyup", (e) =>
      @generateUrls()
      @addUrlsToPage(@urls) if @getCurrentTab() is 'all'
  ...
  clearAllFields: ->
    $("#menu-inputs input, #menu-inputs textarea").each -> $(this).val("")
    $("#menu-inputs input[type=checkbox]").each -> $(this).attr("checked", null)
  ...
  # Bind tooltips to elements (using bootstrap tooltips)
  bindTooltips: ->
    defaultOptions =
      container: 'body'
      placement: 'top'
  ...
  # Returns the tab that is currently active in the interface.
  getCurrentTab: ->
    if $('#tab-config-xml').hasClass('active')
      'config-xml'
    else
      'all'
